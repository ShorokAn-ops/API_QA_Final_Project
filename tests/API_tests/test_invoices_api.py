...
class TestInvoicesAPI(unittest.TestCase):
    ...
    def test_get_invoices_validation_limit_not_int(self):
        r = self.client.get("/invoices?limit=abc")
        self.assertEqual(r.status_code, 422)

    def test_get_invoices_validation_include_items_not_bool(self):
        r = self.client.get("/invoices?include_items=notabool")
        self.assertEqual(r.status_code, 422)
