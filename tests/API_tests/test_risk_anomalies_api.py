...
class TestRiskAnomaliesAPI(unittest.TestCase):
    ...
    def test_anomalies_validation_limit_negative(self):
        r = self.client.get("/risk/anomalies?limit=-5")
        self.assertEqual(r.status_code, 422)

    def test_anomalies_validation_limit_too_high(self):
        r = self.client.get("/risk/anomalies?limit=501")
        self.assertEqual(r.status_code, 422)

    def test_anomalies_validation_limit_not_int(self):
        r = self.client.get("/risk/anomalies?limit=abc")
        self.assertEqual(r.status_code, 422)
