...
class TestDashboardSummaryAPI(unittest.TestCase):
    ...
    def test_dashboard_summary_validation_limit_too_large(self):
        r = self.client.get("/dashboard/summary?limit=1000000")
        self.assertEqual(r.status_code, 422)

    def test_dashboard_summary_validation_limit_negative(self):
        r = self.client.get("/dashboard/summary?limit=-5")
        self.assertEqual(r.status_code, 422)
